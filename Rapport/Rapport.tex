...
%todo: level <=> player <= pour pouvoir se téléporter puisque c'est level qui a le "board"
...
\end{enumerate} %todo: uniformiser \textit{} ou \og \fg mais choisir une "convention" => \textit{Class#Method} ? (=javadoc)
$1$ a simplement été réalisé par l'ajout d'un champ \og lives \fg \, dans la classe Player et en faisant en sorte que, lors de la collision avec un fantôme, Pac-man perde une vie plutôt que de mourir (appel à \og loseLife() \fg \, plutôt qu'à \og setAlive(false) \fg).
De la même manière, la réalisation de $2$ a pu se faire rapidement, en ajoutant une méthode \og checkNewLifeThreshold(int) \fg , appelée à chaque fois que le joueur reçoit des points. Cette méthode se contente de vérifier si le seuil a été passé et ajoute une vie au joueur le cas échéant.
L'ajout $3$ a demandé plus de réflexion quant à la manière de l'implémenter. En effet, pour permettre une téléportation \og safe \fg , il est nécessaire d'avoir connaissance du niveau (le \og Board \fg), alors que le joueur est, en l'état, le seul à être au courant de sa mort.
Il a été choisi ici d'implémenter un design pattern \textit{Observer}, via une interface \textit{PlayerListener}, qui ne contient qu'une méthode \og onPlayerLoseLife(Player) \fg \, mais qui pourrait être facilement étendue (pour par exemple permettre l'affichage d'une notification lorsque le joueur reçoit une vie).
Via l'implémentation de \textit{PlayerListener}, \textit{Level} est capable de réagir à la mort du joueur et, puisqu'il possède une référence vers le \textit{Board}, de le téléporter. L'énoncé demande une téléportation \textit{aléatoire} à une distance de 4 cases de tout fantôme, il sera fait utilisation de la distance Manhattan\footnote{\url{https://en.wiktionary.org/wiki/Manhattan_distance}}. Lorsque Pac-man meurt, une liste des cases \og possibles \fg \, est récupérée via la méthode \textit{getPossibleSquares()} et le joueur est effectivement transporté aléatoirement sur l'une de celles-ci. Afin de déterminer si une case est \og possible \fg , on s'assure qu'elle soit accessible (qu'elle ne corresponde pas à un mur par exemple) et \og safe \fg , c'est-à-dire qu'aucun fantôme ne soit à portée.
Pour ce faire, pour toute case accessible, on vérifie les occupants des cases \og voisines \fg \, en s'assurant qu'elles ne contiennent pas de fantôme. En réalité, puisque les cases situées à une distance Manhattan d'une case donnée forment une sorte de losange, on itère sur le rectangle des cases situées à une distance horizontale et verticale inférieure à 4, en filtrant ensuite les cases trop éloignées en distance Manhattan. %todo: img pr clarifier
$4$ était également problématique puisque \textit{Level} et \textit{Game} étaient les seuls à être au courant de la réussite d'un niveau, alors que les méthodes permettant le chargement d'un niveau (via \textit{MapParser}) se trouvaient dans \textit{Launcher}. La solution logique semblait être de déplacer les méthodes appelant \textit{MapParser} de \textit{Launcher} vers \textit{Game} mais il a été décidé d'attendre la fusion des différentes extensions individuelles pour réaliser ce changement. Nous voulions en réalité éviter les conflits potentiels lors de la fusion puisque ce \og transfert \fg de méthodes demande un nombre important de modifications. En attendant la fusion, on a simplement fait en sorte que \textit{Game} aie la référence vers une instance de \textit{Launcher}.
...
Pour les ajouts $4$ et $5$, il a été nécessaire d'ajouter des fichiers de niveaux supplémentaires. Puisque la démarche était fastidieuse, une fonctionnalité \og bonus \fg \, a été implémentée : la génération de fichiers de niveaux au format Pac-man à partir d'une image. En effet, en attribuant une couleur à chaque type d'élément du jeu,  une image peut être convertie en un fichier texte équivalent et \og lisible \fg \, par \textit{MapParser}. Cela permet de créer des niveaux rapidement avec n'importe quel logiciel d'édition d'image. Cela permet d'utiliser des fonctionnalités comme le \og remplissage \fg d'une zone, qui évite donc d'entrer le même caractère un grand nombre de fois (on obtient également un rendu visuel plus clair que le fichier texte équivalent).
%todo: img pr clarifier
