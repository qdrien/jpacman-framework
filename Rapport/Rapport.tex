...
Avant de terminer cette analyse au niveau des métriques, le logiciel \textit{SonarQube} (voir section 5 : analyse de l'évolution du logiciel) a été utilisé pour avoir les résultats d'un autre logiciel d'analyse de qualité du code. Nous pouvons ainsi encore une fois comparer les résultats obtenus (pour certaines métriques) pour la version finale avec le version initiale. Il est important de spécifier que les métriques utilisés par ce logiciel ne sont pas nécessairement les mêmes que celles utilisées précédemment, ce qui implique que les résultats ne sont pas les mêmes que lors des analyses précédentes.
...
Le logiciel \textit{SonarQube} peut également effectuer une comparaison globale des deux versions dont on a reporté les résultats dans la figure~\ref{sonarComparaison}.
...
les opérations de lecture et d' écriture de fichier spécifient maintenant le \og charset \fg() et les concaténations de chaînes de caractères ont été remplaçées par des \og StringBuilder.append() \fg().%todo: une raison particulière pour les "()" partout ?
...
\begin{itemize}
	...
	\item La classe \mbox{\textit{AILevel}} jouit d' une couverture de 66,7\% au niveau des classes, 61,5\% au niveau des méthodes et 64,4\% au niveau des lignes. %J'ignore comment justifier ou même caractériser le contenu non couvert.
	...
	%Note: ici, je suis tombé à court de façons différentes de dire "à une couverture de".
	...
	\item La classe \mbox{\textit{PacManhattanAI}} a une couverture de 100\% au niveau des classes, 92,9\% au niveau des méthodes et 81,7\% au niveau des lignes. %J'ignore comment justifier ou même caractériser le contenu non couvert.
\end{itemize}
...
%Note: Latex indiqué que "réinstauré" est fautif: il n'en est rien.  Pour court-circuité, pê trouver un remplaçement.
...
Cette section fournit une analyse de l'évolution de la qualité du logiciel. Pour effectuer cette analyse, le logiciel \textit{SonarQube} a été utilisé. \textit{SonarQube} est un outil tout-en-un, indépendant des IDEs, qui permet de calculer et monitorer la qualité logicielle, de voir l'évolution entre les versions du logiciel et aussi de comparer ces versions entre elles très facilement. C'est un outil open source qui a pour objectif de fournir une analyse compète de la qualité d'une application en fournissant de nombreuses statistiques (ou métriques) sur un projet. Ces données permettent ainsi d'évaluer la qualité du code, et d'en connaître l'évolution au cours du développement.


Dans cette section, cet outil est utilisé principalement pour effectuer l'analyse de l'évolution du logiciel après l'ajout de trois fonctionnalités supplémentaires par rapport à la version d'origine.
